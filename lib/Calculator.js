...
function Calculator(tokens, env) {
  // выполнить разбивку списка команд-меток

  this.tokens = tokens; 
  this.env = env;
}

Calculator.evaluate = function (expr, env) {
  ...
  if (node.type === "number") {
    ...
    switch (node.value) {
      ...
      case "/":
        return left / right;
      default:
        throw "Неверный ввод: " + node.value;
    }
  }
};
...
Calculator.prototype._handleParenExpressions = function (tokenArr) {
  // найти сначала левую скобку, затем закрывающую правую
